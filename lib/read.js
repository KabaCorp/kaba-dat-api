...
const {maybe, toKabaError, toValidEncoding, massageMetadataOutput} = require('./common')
  ...
  return maybe(cb, async function () {
    ...
    return new Promise((resolve, reject) => {
      archive.readFile(name, opts, (err, data) => {
        if (err) reject(toKabaError(err, 'readFile'))
        else {
          if (parseJson) {
            try { data = JSON.parse(data) }
            catch (e) { reject(toKabaError(e, 'readFile')) }
          }
          resolve(data)
        }
      })
  ...
  return maybe(cb, function () {
    ...
    return new Promise((resolve, reject) => {
      archive.readdir(name, {recursive, includeStats, noMounts: true}, (err, names) => {
        if (err) reject(toKabaError(err, 'readdir'))
        else {
          if (includeStats) {
...
function createReadStream (archive, name, opts) {
  ...
  stream.on('error', err => pass.destroy(toKabaError(err, 'createReadStream')))
