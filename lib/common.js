...
const {NotFoundError, NotAvailableError, EntryAlreadyExistsError, DestDirectoryNotEmpty, ArchiveNotWritableError} = require('kaba-error-constants')
...
function toKabaError (err, info) {
  ...
  } else if (err.toString().indexOf('Block not available') !== -1 || err.toString().indexOf('Seek not available') !== -1) {
