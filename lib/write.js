...
const {maybe, toKabaError, toValidEncoding, encodeMetadata, ensureParentDir} = require('./common')
const {VALID_PATH_REGEX} = require('./const')
const {
  InvalidEncodingError,
  InvalidPathError,
  EntryAlreadyExistsError
} = require('kaba-error-constants')
  ...
  return maybe(cb, async function () {
    ...
    return new Promise((resolve, reject) => {
      archive.writeFile(name, data, writeOpts, err => {
        if (err) reject(toKabaError(err, 'writeFile'))
        else resolve()
      })
    })
  })
  ...
  return maybe(cb, async function () {
    ...
    return new Promise((resolve, reject) => {
      archive.mkdir(name, err => {
        if (err) reject(toKabaError(err, 'mkdir'))
        else resolve()
      })
    })
  })
  ...
  return maybe(cb, async function () {
    ...
    return new Promise((resolve, reject) => {
      archive.symlink(srcname, dstname, err => {
        if (err) reject(toKabaError(err, 'symlink'))
        else resolve()
      })
    })
  })
  ...
  return maybe(cb, async function () {
    ...
    return new Promise((resolve, reject) => {
      archive.updateMetadata(path, metadata, err => {
        if (err) reject(toKabaError(err, 'updateMetadata'))
        else resolve()
      })
    })
  })
  ...
  return maybe(cb, async function () {
    return new Promise((resolve, reject) => {
      archive.deleteMetadata(path, keys, err => {
        if (err) reject(toKabaError(err, 'deleteMetadata'))
        else resolve()
      })
    })
  })
  ...
  if (sourceStat.mount) {
    ...
    return new Promise((resolve, reject) => {
      pump(
        ...
        err => {
          if (err) reject(toKabaError(err, 'createReadStream/createWriteStream'))
          else resolve()
        }
      )
    })
